...
class Cart {
    ...
    //метод добавляет новый товар в корзину, или увеличивает количество позиций по добавленному ранее
    addToCart(product) {
...
function generateCartView(itemsCart) {
    ...
    function generateCartTable(cart, cartTable) {
        ...
        cart.products.forEach(item => {
            tableHTML += `<tr> <td>${item.name}</td> <td>${item.count}</td> <td>${item.price}</td> </tr>`;
        });
        tableHTML += `<tr> <th>Итого:</th> <th>${cart.countProducts}</th> <th>$${cart.fullPrice}</th> </tr>`;
